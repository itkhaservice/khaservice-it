...
<head>
    ...
    <style>
        ...
        .card { background: #fff; border-radius: 20px; padding: 20px; box-shadow: 0 10px 25px rgba(0,0,0,0.05); margin-bottom: 20px; }
        ...
        .btn-view-report { width: 100%; padding: 12px; border: 2px solid var(--primary); background: #fff; color: var(--primary); border-radius: 12px; font-weight: 700; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; margin-bottom: 20px; }

        .section-label { font-size: 0.8rem; font-weight: 800; color: var(--gray); text-transform: uppercase; margin-bottom: 10px; display: block; }
        ...
        .role-sub { font-size: 0.7rem; color: var(--gray); display: block; margin-top: 4px; }
        ...
        .input-group input { width: 100%; padding: 14px; border: 2px solid #f1f5f9; border-radius: 12px; font-size: 1rem; font-weight: 600; box-sizing: border-box; }
...
<body>
    <div class="main-container">
        ...
        <div class="card">
            ...
            <button class="btn-view-report" onclick="toggleReport(true)"><i class="fas fa-file-invoice"></i> Xem nội dung chi tiết</button>
        </div>
        ...
        <div class="role-grid">
            <div class="role-card <?php echo !empty($log['it_signature']) ? 'signed' : ''; ?>" id="role-1" onclick="selectRole(1, '<?php echo addslashes($log['it_name']); ?>')">
                ...
                <span class="role-sub"><?php echo htmlspecialchars($log['it_name']); ?></span>
                <?php if($log['it_signature']): ?><span class="signed-text">ĐÃ KÝ XÁC NHẬN</span><?php endif; ?>
            </div>
            <div class="role-card <?php echo !empty($log['customer_signature']) ? 'signed' : ''; ?>" id="role-2" onclick="selectRole(2, '<?php echo addslashes($log['client_name'] ?? ''); ?>')">
                ...
                <span class="role-sub">BQL Dự án / Đại diện</span>
                <?php if($log['customer_signature']): ?><span class="signed-text">ĐÃ KÝ XÁC NHẬN</span><?php endif; ?>
            </div>
        </div>

        <div class="input-group" id="name-group">
            <span class="section-label">2. Xác nhận họ tên</span>
            <input type="text" id="signer_name" placeholder="Nhập đầy đủ họ tên của bạn">
        </div>
    ...
    <!-- PREVIEW MODAL -->
    <div id="report-modal">
        <div class="sig-top-bar" style="background:#fff;">
            <div style="font-weight:800; color:var(--primary);">NỘI DUNG PHIẾU</div>
            <button onclick="toggleReport(false)" style="background:var(--primary); color:#fff; border:none; padding:8px 20px; border-radius:8px; font-weight:700;">ĐÓNG</button>
        </div>
    ...
    <div id="modal-confirm" class="modal-blur">
        <div class="modal-box">
            ...
            <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px;">
                <button class="btn-clear" style="margin:0;" onclick="hideModal('modal-confirm')">KÝ LẠI</button>
                <button class="btn-done" style="margin:0;" onclick="submitSignature()">ĐỒNG Ý</button>
            </div>
        </div>
    </div>
    ...
    <div id="modal-success" class="modal-blur">
        <div class="modal-box">
            ...
            <h2>Thành công!</h2>
            <p style="color:var(--gray); margin-bottom:25px;">Chữ ký của bạn đã được ghi nhận và lưu vào hệ thống.</p>
            <button class="btn-done" style="width:100%;" onclick="window.location.reload()">XEM BIÊN BẢN</button>
        </div>
    </div>
    ...
    <script>
        ...
        function selectRole(idx, name) {
            ...
            document.getElementById('signer_name').value = (idx === 1) ? name : (name || '');
            if (idx === 2) document.getElementById('signer_name').focus();
        }

        function startSigning() {
            if (!selectedRole) return alert("Vui lòng chọn người ký (IT hoặc Khách hàng)");
        ...
        function toggleReport(show) {
            ...
            if (show) {
                ...
                iframe.src = 'render_maintenance_preview.php?token=<?php echo $token; ?>';
            } else {
