...
if (!isset($_SESSION['role']) || ($_SESSION['role'] !== 'admin' && $_SESSION['role'] !== 'it')) {
    set_message("Bạn không có quyền thêm mới!", "error");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("INSERT INTO car_system_configs (project_id, server_ip, db_name, folder_path, username, password) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->execute([$project_id, $server_ip, $db_name, $folder_path, $username, $password]);
            
            set_message("Thêm cấu hình thành công!", "success");
            echo '<script>window.location.href = "index.php?page=car_systems/list";</script>';
            exit;
        } catch (PDOException $e) {
...
<div class="page-header">
    <h2><i class="fas fa-plus-circle"></i> Thêm Cấu hình Mới</h2>
    <div class="header-actions">
        <a href="index.php?page=car_systems/list" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> Quay lại</a>
        <button type="submit" form="form-add" class="btn btn-primary"><i class="fas fa-save"></i> Lưu lại</button>
    </div>
</div>

<div class="card">
    <div class="card-body">
        <form id="form-add" method="POST" action="" class="form-grid">
                    ...
                    <input type="text" id="project_search" class="form-control" placeholder="Gõ tên dự án để tìm kiếm..." value="<?php 
                        if (isset($_POST['project_id'])) {
                            foreach($projects as $p) {
                                if($p['id'] == $_POST['project_id']) { echo htmlspecialchars($p['ten_du_an']); break; }
                            }
                        }
                    ?>" autocomplete="off" required>
                    <button type="button" class="btn-clear" id="btn-clear-project" title="Xóa chọn" style="<?php echo (isset($_POST['project_id']) && $_POST['project_id']) ? 'display:block' : 'display:none'; ?>"><i class="fas fa-times"></i></button>
                    <input type="hidden" name="project_id" id="project_id" value="<?= htmlspecialchars($_POST['project_id'] ?? '') ?>">
            ...
            <div class="form-group">
                ...
                <input type="text" name="server_ip" id="server_ip" class="form-control" value="<?= htmlspecialchars($_POST['server_ip'] ?? '') ?>">
            </div>

            <div class="form-group">
                <label for="db_name" class="form-label">Tên Cơ sở dữ liệu (Database)</label>
                <input type="text" name="db_name" id="db_name" class="form-control" value="<?= htmlspecialchars($_POST['db_name'] ?? '') ?>">
            </div>

            <div class="form-group full-width">
                <label for="folder_path" class="form-label">Đường dẫn thư mục cài đặt</label>
                <input type="text" name="folder_path" id="folder_path" class="form-control" placeholder="Ví dụ: D:\Setup\Parking..." value="<?= htmlspecialchars($_POST['folder_path'] ?? '') ?>">
            </div>

            <div class="form-group">
                <label for="username" class="form-label">Tài khoản quản trị</label>
                <input type="text" name="username" id="username" class="form-control" value="<?= htmlspecialchars($_POST['username'] ?? '') ?>">
            </div>

            <div class="form-group">
                <label for="password" class="form-label">Mật khẩu</label>
                <div class="input-group-password" style="width: 100%;">
                    <input type="text" name="password" id="password" class="form-control" style="width: 100%;" value="<?= htmlspecialchars($_POST['password'] ?? '') ?>">
                </div>
                <small class="form-text text-muted">Lưu ý: Mật khẩu được lưu để tham khảo nội bộ.</small>
            </div>
        </form>
    </div>
</div>