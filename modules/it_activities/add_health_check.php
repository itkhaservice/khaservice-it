...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_health_check'])) {
    ...
    $checked_by = $_POST['checked_by'] ?: $_SESSION['user_id']; // Lấy từ form hoặc mặc định
    ...
    try {
        ...
        $pdo->commit();
        set_message("success", "Đã lưu báo cáo thành công!");
        echo "<script>window.location.href = 'index.php?page=it_activities/list';</script>";
        exit;
    } catch (Exception $e) { $pdo->rollBack(); set_message("error", "Lỗi: " . $e->getMessage()); }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['quick_add_device'])) {
    try {
        ...
        $stmt->execute([$_POST['q_ten_thiet_bi'], $_POST['q_nhom_thiet_bi'], $_POST['q_loai_thiet_bi'], $_POST['q_ma_tai_san'], $_POST['project_id'], $_POST['q_parent_id'] ?: null, $_POST['q_model'] ?: null, $_POST['q_serial'] ?: null]);
        set_message("success", "Đã thêm nhanh thiết bị!");
...
$potential_parents = [];
if ($project_id) {
    $stmt = $pdo->prepare("SELECT id, ten_thiet_bi, nhom_thiet_bi, loai_thiet_bi, ma_tai_san, parent_id FROM devices WHERE project_id = ? AND deleted_at IS NULL ORDER BY nhom_thiet_bi, parent_id ASC, ten_thiet_bi ASC");
    ...
    foreach ($roots as $group => $root_list) {
        foreach ($root_list as $root) {
            ...
            if (isset($children[$root['id']])) {
                foreach ($children[$root['id']] as $child) $tree_by_group[$group][] = ['item' => $child, 'level' => 1];
            }
        }
    }
}
        ...
        <div class="col-md-4">
            <div class="card mb-3 shadow-sm">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select name="checked_by" class="form-control" required>
                            <?php foreach ($it_staff as $staff): ?>
                                <option value="<?= $staff['id'] ?>" <?= $_SESSION['user_id'] == $staff['id'] ? 'selected' : '' ?>><?= htmlspecialchars($staff['fullname']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="form-group mb-3">
                    ...
                    <div class="form-group"><label>Ghi chú tổng quan</label><textarea name="summary_notes" class="form-control" rows="4"></textarea></div>
                </div>
            </div>
            <?php if ($project_id): ?><div class="card shadow-sm border-success text-center p-3"><p class="mb-2 small text-muted">Thiết bị còn thiếu?</p><button type="button" class="btn btn-primary btn-sm w-100" onclick="openQuickAddModal()"><i class="fas fa-bolt"></i> THÊM NHANH THIẾT BỊ</button></div><?php endif; ?>
        </div>
        
        <div class="col-md-8">
            <div class="health-check-container">
                <?php if (!$project_id): ?><div class="alert alert-info">Vui lòng chọn <strong>Dự án</strong> để liệt kê danh sách thiết bị.</div>
                <?php elseif (empty($tree_by_group)): ?><div class="alert alert-warning">Chưa có thiết bị nào được quản lý.</div>
                <?php else: ?>
                                ...
                                <tbody>
                                    ...
                                    ?>
                                        <tr class="<?= $lvl == 0 ? 'row-root' : 'row-child' ?>">
                                            ...
                                            <td><select name="status[]" class="select-styled" onchange="updateStatusColor(this)"><option value="Đang sử dụng" class="status-inuse">Đang dùng</option><option value="Không sử dụng" class="status-notinuse">Không dùng</option></select></td>
                                            <td><select name="health_status[]" class="select-styled"><option value="good">Tốt</option><option value="warning">Cảnh báo</option><option value="broken">Hỏng</option></select></td>
                                            <td><input type="number" name="quantity[]" value="1" min="0" class="input-styled text-center"></td>
                                            <td><input type="text" name="cause[]" class="input-styled mb-1" placeholder="Nguyên nhân..."><input type="text" name="notes[]" class="input-styled" placeholder="Ghi chú..."></td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
...
<div id="quickAddModal" class="modal-custom">
    <div class="modal-content-custom">
        <div class="modal-header-custom"><h3><i class="fas fa-bolt"></i> Thêm nhanh Thiết bị</h3><span class="close-modal" onclick="closeQuickAddModal()">&times;</span></div>
        <form action="" method="POST" id="quick-add-form">
            <input type="hidden" name="quick_add_device" value="1"><input type="hidden" name="project_id" id="q_project_id" value="<?= $project_id ?>">
