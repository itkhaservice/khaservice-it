<?php
// modules/devices/edit.php
$device_id = $_GET['id'] ?? null;

if (!$device_id) {
    set_message('error', 'Không tìm thấy ID thiết bị!');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_device'])) {
    ...
    if (empty($ma_tai_san) || empty($ten_thiet_bi)) {
        set_message('error', 'Mã tài sản và tên thiết bị là bắt buộc!');
    } else {
        try {
            ...
            set_message('success', 'Cập nhật thiết bị thành công!');
            echo "<script>window.location.href = 'index.php?page=devices/view&id=$device_id';</script>";
            exit;
...
<div class="page-header">
    ...
    <div class="header-actions">
        ...
        <button type="submit" form="edit-device-form" name="save_device" class="btn btn-primary"><i class="fas fa-save"></i> Lưu Thay đổi</button>
    </div>
</div>

<form action="" method="POST" id="edit-device-form" class="edit-layout">
...
.card-header-custom h3 i { color: #fff; background: var(--gradient-primary); padding: 8px; border-radius: 8px; font-size: 1rem; box-shadow: 0 4px 6px -1px rgba(36, 162, 92, 0.3); }
.form-row { display: flex; gap: 20px; }
.form-group.half { flex: 1; }
.input-highlight { background-color: #f8fafc; border-color: #cbd5e1; color: var(--primary-dark-color); font-weight: 600; }

/* Responsive */
@media (max-width: 992px) { .edit-layout { grid-template-columns: 1fr; } }
@media (max-width: 768px) {
    .page-header { flex-direction: column; align-items: flex-start; gap: 15px; }
    .header-actions { width: 100%; display: flex; flex-direction: row !important; flex-wrap: wrap; gap: 8px; }
    .header-actions .btn, .header-actions a { flex: 1 1 auto; min-width: calc(50% - 8px); justify-content: center; height: 40px; font-size: 0.85rem; }
...
.btn-clear { position: absolute; right: 10px; top: 50%; transform: translateY(-50%); background: none; border: none; color: #94a3b8; cursor: pointer; padding: 5px; display: none; }
.searchable-select-container:hover .btn-clear, .search-input:focus + .btn-clear { display: block; }
